		...
		for(int i = y; i < 256; i++)
		{
			if(world.getBlock(x, i, z).isWood(world, x, i, z))
			{
				...
				if(!world.isRemote && world.getGameRules().getGameRuleBooleanValue("doTileDrops")) // si la règle de jeu drop est activé
				{
					...
					EntityItem entityitem = new EntityItem(world, (double)x + d0, (double)i + d1, (double)z + d2, new ItemStack(world.getBlock(x, i, z), 1, world.getBlockMetadata(x, i, z))); // instancie une nouvelle entité item avec les coordonnées + l'id et le metadata du bois
